...
def playerInput(board):
    while True:
        if currentPlayer == "X":
            inp = int(input("Enter a number 1-9 Player (X): "))
        else:
            inp = int(input("Enter a number 1-9 Player (0): "))
        if inp >= 1 and inp <= 9 and board[inp-1] == "-":
            board[inp-1] = currentPlayer
            break
        else:
            if currentPlayer == "X":
                print("Oops! Try again Player (X)!")
            else:
                print("Oops! Try again Player (0)!")
            printBoard(board)
...
def checkTie(board):
    ...
    if "-" not in board:
        ...
        gameRunning = False
def checkWin():
    if checkDiagonal(board) or checkHorizontal(board) or checkColumn(board):
